...
Current coverage: **Portugal** (live), Spain and France (pipeline ready, same
...
actual country polygon — no bounding-box rectangles. All raster sources are
warped onto a common ~200m grid per region, then aggregated per hexagon.

## Data sources

Everything is public and keyless — no accounts, tokens or paid tiers anywhere
in the pipeline.
...
| [ERA5 via Open-Meteo](https://open-meteo.com/) (primary), [NASA POWER](https://power.larc.nasa.gov/) (fallback) | Daily max temperature, min relative humidity, max wind + gusts, precipitation, ET0 on a 0.25–0.5° grid | ~25–50 km | New days appended daily |
| [EFFIS](https://forest-fire.emergency.copernicus.eu/) burnt-area perimeters (Copernicus) | Fire polygons with dates — the training labels and the historical-fires layer | per-fire polygons, 2016→present | Refetched daily |
...
| [OpenStreetMap](https://www.geofabrik.de/) power lines (Geofabrik extracts) | Transmission + distribution line geometry, voltage, operator; split into ≤5 km corridor segments, named by nearest town | vector | Static (re-ingest on demand) |
...
touches the cell). Ignition is a rare event — the base rate is ~0.1–0.15% —
...
mean/10th-percentile NDVI, NDMI, NDVI anomaly vs the cell's own monthly
...
natural next experiment — the honest bar it has to beat is below.
...
Read PR-AUC against the 0.15% base rate: 0.014 is ~10× lift over random, and
2025 — the big fire season — is where the model is strongest. Accuracy is not
...
forecast — a production system would swap in NWP forecasts, backtests are
